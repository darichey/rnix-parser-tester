...
(See the [`cli` crate readme](./crates/cli/README.md) for full documentation)
...
* Accepts Nix store paths between angle brackets
...
* The reference implementation is written in C++. Rust has limited interop with C++, so a [basic C-compatible wrapper](./crates/ref-impl-parser/reference_to_json/README.md) was written to do the bulk of the reference impl-related work on the C++ side.
...
* rnix-parser's typed AST leaves a lot to be desired from a usability standpoint. We created [our own](./crates/rnix-ast/README.md) which is easier to work with for our use case. 
...
### Normalization
Normalization is the process of taking the AST produced by rnix-parser and converting it to the [structure used by the reference impl](./crates/normal-ast/README.md). For example, consider the following expression:
...
The [`rnix-normalize`](./crates/rnix-normalize/README.md) crate explains in detail all of the normalization rules.
